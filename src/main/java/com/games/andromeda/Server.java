...
public abstract class Server<C extends Connection, CC extends ClientConnector<C>> extends Thread {
	...
	//protected abstract void onMoveShipClientMessage();
...
//			android.os.Process.setThreadPriority(android.os.Process.THREAD_PRIORITY_DEFAULT);  // TODO What ThreadPriority makes sense here?
			...
			while (!Thread.interrupted() && this.mRunning.get() && !this.mTerminated.get()) {
				try {
					...
					clientConnector.registerClientMessage(FLAG_MESSAGE_CLIENT_SHOW, MoveShipClientMessage.class, new IClientMessageHandler<C>() {
						@Override
						public void onHandleMessage(ClientConnector<C> clientConnector, IClientMessage iClientMessage) throws IOException {
							//Log.wtf("мб","мб");
							try  {
								float x = 0.5f, y = 0.5f;
	...
	public static interface IServerListener<S extends Server<?, ?>> {
		...
		//public void onMoveShipClientMessage(final S pServer);
	}
}
